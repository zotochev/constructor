...
from .calculator import Loan, NotReadyToCalculate, PAYMENT_FIELDS_NAMES
...
class LoanPlugin(APlugin):
	...
	def __init__(self, page: Page, event_system):
		...
		self.payments_table: ListView = None
		self.payments_table_header: ListView = None
		self.payments_table_container = None
		self.build_table()
	...
	def __on_change(self, setter: Callable, value: Any):
		try:
			...
			self.payments_table_container.visible = False
	...
	def __render_loan(self):
		...
		del self.payments_table.controls[1:]
		rows = []

		for n in range(self.calculator.number_of_payments()):
			payment = self.calculator.get_payment(n)
			rows.append(Text(render_header([f'{x:.2f}' for x in payment], TABLE_COLUMN_WIDTH), font_family=TABLE_FONT, selectable=True))

		self.payments_table.controls.extend(rows)
		self.payments_table_container.visible = True
